...
const insecureAgent = new https.Agent({ rejectUnauthorized: false, keepAlive: true, maxSockets: 32 });
...
function requestText(url, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        // абсолютный таймаут: req.setTimeout не срабатывает, пока TCP/TLS не установился
        if (timeoutMs > 0) {
            timer = setTimeout(() => {
                const err = new Error(`Таймаут ${timeoutMs}мс: ${u.hostname}`);
                req.destroy();
                finish(reject, err);
            }, timeoutMs);
        }
...
export async function httpsFetchFollow(url, options = {}) {
    ...
    for (let hop = 0; hop <= maxRedirects; hop++) {
        ...
        const res = await requestText(currentUrl, {
            method,
            headers,
            body,
