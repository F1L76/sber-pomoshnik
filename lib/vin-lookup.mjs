...
function buildLookupPayload({ vins = [], plates = [], queries = [], text = "", plate = "" } = {}) {
    if (Array.isArray(queries) && queries.length) {
        return { queries: queries.map((v) => String(v).trim()).filter(Boolean).slice(0, MAX_VINS) };
    }
    if (Array.isArray(plates) && plates.length) {
        return { plates: plates.map((v) => String(v).trim()).filter(Boolean).slice(0, MAX_VINS) };
    }
    if (Array.isArray(vins) && vins.length) {
        return { vins: vins.map((v) => String(v).trim()).filter(Boolean).slice(0, MAX_VINS) };
    }
    if (plate) {
...
function assertLookupCount(payload) {
    const n = countVins(payload);
    if (!n) throw new Error("Укажите VIN или госномер");
    if (n > MAX_VINS) throw new Error(`Не более ${MAX_VINS} номеров за один запрос`);
    return n;
}

export async function lookupVins(opts = {}) {
    const payload = buildLookupPayload(opts);
    const n = assertLookupCount(payload);
    return runVinLookup(findPython(), payload, lookupTimeoutMs(n));
}

/** NDJSON stream: пишет в writable готовые строки CLI (type=result|done|error). */
export function lookupVinsNdjson(opts = {}, writable) {
    const payload = { ...buildLookupPayload(opts), stream: true };
    const n = assertLookupCount(payload);
    const python = findPython();
    const timeoutMs = lookupTimeoutMs(n);
...
export async function checkVinHealth() {
    ...
    // ponytail: не ходить в drom на health — иначе 10–60 с и ложные «тормоза» UI
    return { ok: true, python, source: "cli" };
}
