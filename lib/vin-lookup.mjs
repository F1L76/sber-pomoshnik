...
function lookupTimeoutMs(vinCount) {
    if (vinCount <= 1) return 120_000;
    return Math.min(900_000, 30_000 + vinCount * 15_000);
}
...
export function countVins({ vins = [], text = "" } = {}) {
...
export async function lookupVins({ vins = [], text = "" } = {}) {
    const payload =
        Array.isArray(vins) && vins.length
            ? { vins: vins.map((v) => String(v).trim()).filter(Boolean).slice(0, MAX_VINS) }
            : { text: String(text || "") };

    const n = countVins(payload);
    if (!n) {
        throw new Error("Укажите хотя бы один VIN");
    }
    if (n > MAX_VINS) {
        throw new Error(`Не более ${MAX_VINS} VIN за один запрос`);
    }
