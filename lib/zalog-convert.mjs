...
function runPython(python, args) {
    return new Promise((resolve, reject) => {
        ...
        // ponytail: не += Buffer в string — кириллица ломается на границе чанков → «Ба��ка»
        const outChunks = [];
        const errChunks = [];
...
export async function readZalogUpload(req) {
    ...
    if (!buffer.length) {
        throw new Error(
            "Пустое тело загрузки (multipart). Обновите страницу (Cmd+Shift+R) и повторите — будет использован запасной способ."
        );
    }
    const parts = parseMultipartFormData(buffer, contentType);
    const pdf = parts.get("pdf") || parts.get("file") || parts.get("conclusion");
    const xlsx =
        parts.get("xlsx") || parts.get("xls") || parts.get("objects") || parts.get("spreadsheet");
    if (!pdf?.data?.length || !xlsx?.data?.length) {
        const found = [...parts.keys()].join(", ") || "нет полей";
        throw new Error(
            `Загрузите оба файла: PDF заключение и XLSX перечень залога (на сервере: ${found}, bytes=${buffer.length})`
        );
    }
...
let probeCache = { ts: 0, value: null };
...
export async function probeZalogPythonDepsCached() {
    const now = Date.now();
    if (probeCache.value?.ok && now - probeCache.ts < 120_000) return probeCache.value;
    const value = await probeZalogPythonDeps();
    probeCache = { ts: now, value };
    return value;
}
...
export function getZalogConverterHealth() {
    ...
    const hasRiskRag = fs.existsSync(path.join(ROOT, "data", "risk_minimization_rag.xlsx"));
    return {
        ok: hasCli && hasPackage,
        python,
        hasPython: fs.existsSync(python) || python === "python3",
        pythonDepsBundled: hasBundledDeps,
        riskMinimizationRag: hasRiskRag,
        mode: "rule-based",
        gigachat: false,
        package: hasPackage
    };
