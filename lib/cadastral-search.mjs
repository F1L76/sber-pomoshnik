...
/**
 * Потоковый поиск: отдаёт события по мере готовности (НСПД → панорама/площадки → done).
...
export async function* streamCadastralSearch(rawNumber) {
    ...
    yield {
        ...
        data: {
            cadastralNumber,
            rosreestr,
            searchLinks: buildSearchLinks(cadastralNumber, rosreestr.object?.address || "")
        }
    };

    const kadbase = await lookupKadbaseObject(cadastralNumber, {
        address: rosreestr.object?.address || ""
    });
    yield {
        event: "kadbase",
        data: {
    ...
    const avitoLocationId = getAvitoLocationId(cadastralNumber, rosreestr.object?.address || "");
    const context = {
        cadastralNumber,
        address: rosreestr.object?.address || ""
    };
    ...
    const pool = new Map([
        ...
        ["domclick", searchDomclick({ queries: avitoDomclickExact, streetQueries: avitoDomclickStreet })],
        [
            "cian",
            searchCian({
                queries: cianQueries,
