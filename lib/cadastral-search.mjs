...
export async function* streamCadastralSearch(rawNumber) {
    ...
    const pool = new Map([
        ...
        [
            ...
            withDeadline(
                searchAvito({
                    queries: avitoDomclickExact.slice(0, 2),
                    streetQueries: avitoDomclickStreet.slice(0, 1),
                    locationId: avitoLocationId
                }).catch((err) => ({ listings: [], errors: [err?.message || String(err)] })),
                SOURCE_DEADLINE_MS.avito,
                () => ({
                    listings: [],
                    errors: [],
                    unavailable: {
                        source: "avito",
                        reason: "timeout",
                        message: "Авито не ответил вовремя — откройте поиск по ссылке"
                    }
                })
            )
        ],
        [
            ...
            withDeadline(
                ...
                SOURCE_DEADLINE_MS.domclick,
                () => ({ listings: [], errors: ["Домклик не ответил вовремя"] })
            )
        ],
        [
            "cian",
            withDeadline(
                searchCian({
                    queries: cianQueries.slice(0, 3),
                    regionId,
                    objectType: rosreestr.object?.objectType,
                    cadastralNumber
                }).catch((err) => ({ listings: [], errors: [err?.message || String(err)] })),
                SOURCE_DEADLINE_MS.cian,
                () => ({ listings: [], errors: ["Циан не ответил вовремя"] })
            )
        ]
    ]);
