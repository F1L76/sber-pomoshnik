...
function buildRadiusNote(meta) {
    const km = meta?.radiusKm;
    if (km == null || km <= 30) return null;
    return `В радиусе 30 км найдено мало объявлений — показаны аналоги в радиусе до ${km} км.`;
}
...
export async function* streamCadastralSearch(rawNumber) {
    ...
    if (!rosreestr.found) {
        yield { event: "kadbase", data: await loadKadbasePayload(cadastralNumber, address, rosreestr) };
        yield {
            event: "done",
            data: {
