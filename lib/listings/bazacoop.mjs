...
function enrichWithDeadline(listings, deadlineMs) {
    if (!deadlineMs || deadlineMs <= 0 || !listings.length) {
        return Promise.resolve({ listings, completed: true });
    }
    let timer;
    return Promise.race([
        enrichListingsWithSourceUrls(listings, { limit: 20, concurrency: 6 })
...
export async function searchBazacoop({
    ...
    if (listings.length) {
        const enriched = await enrichWithDeadline(listings, enrichDeadlineMs);
