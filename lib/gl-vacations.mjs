...
function surnameKey(fio) {
    return vacationSurname(fio).toLowerCase();
}
...
const BUILTIN_BY_SURNAME = new Map(
    Object.entries(BUILTIN_VACATIONS).map(([k, v]) => [k.replace(/ё/g, "е").toLowerCase(), v])
);

export function isGlEmployee(fio) {
    return BUILTIN_BY_SURNAME.has(surnameKey(fio));
}
...
export function resolveVacations(fio, overrides = {}) {
    ...
    const builtin = BUILTIN_BY_SURNAME.get(surnameKey(fio));
    return builtin ? builtin.map((p) => ({ ...p })) : [];
}
