...
export async function searchDealsByQuarter(rawQuarter, { limit = 10000, year } = {}) {
    ...
    if (isSqliteReady()) {
        const deals = searchDealsFromSqlite(quarter, { year: yearFilter }) || [];
    ...
    for (const datasetMeta of datasets) {
        ...
        const batch = await scanDataset(datasetMeta, quarter);
