...
export function dedupeConclusionDocs(docs) {
    const ranked = [...docs].sort((a, b) => {
        const aTpl = a.collection === "template" ? 0 : 1;
        const bTpl = b.collection === "template" ? 0 : 1;
...
/**
 * Если рядом с топом есть шаблон ГЛ — предпочесть его (ровный ответ playbook).
 */
export function preferTemplateHit(hits) {
    ...
    const preferred = hits.find(
        (h) =>
            (h.collection === "faq"
                || h.collection === "template"
                || (h.sheet && h.sheet !== "ГЛ"))
            && (Number(h.score) || 0) >= topScore - (h.collection === "faq" ? 0.35 : 0.15)
            && (Number(h.score) || 0) >= 1.0
    );
    if (!preferred || preferred === top) return hits;
    return [preferred, ...hits.filter((h) => h !== preferred)];
}

/**
 * verbatim — отдать текст базы; synthesize — GigaChat по top-k.
 * ponytail: пороги по текущим score эталона; при смене ранжирования пересмотреть.
 */
export function chooseAskMode(hits) {
    const list = preferTemplateHit(hits);
    const top = list[0];
    if (!top?.answer?.trim()) return { mode: "empty", hits: list };
    const isTpl =
        top.collection === "faq"
        || top.collection === "template"
        || (top.sheet && top.sheet !== "ГЛ");
    const score = Number(top.score) || 0;
    const len = String(top.answer).trim().length;
    if (top.collection === "faq" && score >= 0.8) {
        return { mode: "verbatim", reason: "faq", hits: list, hit: top };
    }
    if (isTpl && score >= 0.65) {
        return { mode: "verbatim", reason: "template", hits: list, hit: top };
    }
    if (score >= 1.2 && len <= 1000) {
        return { mode: "verbatim", reason: "strong", hits: list, hit: top };
    }
    if (score >= 1.45 && len <= 1600) {
        return { mode: "verbatim", reason: "strong-long", hits: list, hit: top };
    }
    return { mode: "synthesize", reason: "blend", hits: list, hit: top };
}
...
function _selfcheck() {
    ...
    if (modeTpl.mode !== "verbatim") throw new Error("chooseAskMode template");
    const modeSynth = chooseAskMode([
        { sheet: "ГЛ", collection: "case", score: 0.5, answer: "длинный спорный кейс ".repeat(20) }
    ]);
    if (modeSynth.mode !== "synthesize") throw new Error("chooseAskMode synthesize");
