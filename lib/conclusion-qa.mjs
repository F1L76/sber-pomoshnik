...
export function searchConclusionQa(question, opts = {}) {
    ...
    if (!index || index.path !== chunksPath) {
        try {
            ...
            const docs = dedupeConclusionDocs(
                raw
                    .split("\n")
                    .map((l) => l.trim())
                    .filter(Boolean)
    ...
    return {
        ...
        hits: hitsRaw.map(({ doc, score }) => {
            ...
            return {
                id: doc.id,
                sheet,
                collection: doc.collection === "template" ? "template" : "case",
...
export function preferTemplateHit(hits) {
    ...
    const tpl = hits.find(
        (h) =>
            ...
            && (Number(h.score) || 0) >= topScore - 0.3
    );
    if (!tpl || tpl === top) return hits;
    return [tpl, ...hits.filter((h) => h !== tpl)];
}
...
export function chooseAskMode(hits) {
    ...
    const isTpl = top.collection === "template" || (top.sheet && top.sheet !== "ГЛ");
