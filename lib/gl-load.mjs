...
/** Краткий ответ по выгрузке; null — отдать вопрос в GigaChat. */
...
export function assistantDigest(agg) {
    return {
        periodFrom: ymd(agg.qStart),
        periodTo: ymd(agg.today),
        totalClosed: agg.used,
        employees: (agg.employees || []).map((e) => ({
            fio: e.fio,
            apps: e.apps,
            overdue: e.overdue,
            returns: e.returns,
            closedByDay: agg.closedByEmpDay?.[e.fio] || {},
            createdByDay: agg.createdByEmpDay?.[e.fio] || {},
            lines: Object.fromEntries(Object.entries(e.files || {}).map(([k, v]) => [k, v.apps]))
        })),
        lines: agg.lines,
        byHour: agg.byHour,
