...
export function workUnitsOf(appsNormal, appsArb) {
    return appsNormal / PLAN_APPS_PER_WORKDAY + appsArb / PLAN_ARB_PER_WORKDAY;
}
...
export function aggregateEmployeeLoad(tables, now = new Date()) {
    ...
    for (const table of tables) {
        ...
        for (const row of table.rows) {
            ...
            const cur = counts.get(fio) || { apps: 0, appsNormal: 0, appsArb: 0 };
            cur.apps += 1;
            if (arb) {
                cur.appsArb += 1;
                usedArb += 1;
    ...
    const employees = [...counts.entries()]
        .map(([fio, c]) => {
            const workUnits = workUnitsOf(c.appsNormal, c.appsArb);
