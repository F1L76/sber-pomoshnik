...
import { isGlEmployee, resolveVacations, vacationSurname } from "./gl-vacations.mjs";
...
export const SHARE_LIMIT = { overdue: 4, returns: 4, csi: 3 };
...
export function csiBadCount(counts) {
    const c = counts || [];
    return (c[0] || 0) + (c[1] || 0);
}

export function sharePct(n, closed) {
    if (!closed) return null;
    const pct = (Number(n) || 0) / closed * 100;
    return Number.isFinite(pct) ? pct : null;
}

/** Чем выше доля, тем хуже: до половины нормы — ok, до нормы — warn, выше — bad. */
export function shareTone(pct, limit) {
    if (pct == null || !Number.isFinite(pct)) return "";
    if (pct <= limit * 0.5) return "ok";
    if (pct <= limit) return "warn";
    return "bad";
}
...
export function aggregateEmployeeLoad(tables, now = new Date()) {
    ...
    const closedByEmpDay = {};
    const createdByEmpDay = {};
    ...
    for (const table of tables) {
        ...
        for (const row of table.rows) {
            ...
            if (arrived) {
                ...
                const cy = ymd(arrived);
                (createdByEmpDay[fio] ||= {})[cy] = (createdByEmpDay[fio][cy] || 0) + 1;
            }
            const closeYmd = ymd(dt);
            (closedByEmpDay[fio] ||= {})[closeYmd] = (closedByEmpDay[fio][closeYmd] || 0) + 1;
            counts.set(fio, cur);
            used += 1;
        }
    }
...
function fmtRuDay(date) {
    const d = String(date.getDate()).padStart(2, "0");
    const m = String(date.getMonth() + 1).padStart(2, "0");
    return `${d}.${m}.${date.getFullYear()}`;
}

export function dateFromQuestion(text, year) {
    const s = String(text || "")
        .toLowerCase()
        .replace(/ё/g, "е");
    const named = s.match(
    ...
    if (named) {
        return new Date(Number(named[3] || year), RU_MONTHS[named[2]], Number(named[1]));
    }
    ...
    if (dotted) {
        ...
        if (month >= 1 && month <= 12) {
            const yraw = dotted[3];
            const y = !yraw ? year : yraw.length === 2 ? 2000 + Number(yraw) : Number(yraw);
            return new Date(y, month - 1, Number(dotted[1]));
        }
    }
    return null;
}

function employeeFromQuestion(question, employees) {
    const s = String(question || "")
        .toLowerCase()
        .replace(/ё/g, "е");
    const hits = (employees || [])
        .map((e) => ({ e, sur: vacationSurname(e.fio).toLowerCase() }))
        .filter((x) => x.sur.length >= 3 && s.includes(x.sur))
        .sort((a, b) => b.sur.length - a.sur.length);
    return hits[0]?.e || null;
}

function appsPhrase(n) {
    const n10 = n % 10;
    const n100 = n % 100;
    const word =
        n10 === 1 && n100 !== 11 ? "заявка" : n10 >= 2 && n10 <= 4 && (n100 < 10 || n100 >= 20) ? "заявки" : "заявок";
    return `${n} ${word}`;
}

function countOnDay(byEmpDay, fio, dayKey) {
    if (fio) return byEmpDay?.[fio]?.[dayKey] || 0;
    let n = 0;
    for (const days of Object.values(byEmpDay || {})) n += days[dayKey] || 0;
    return n;
}

/** Краткий ответ по выгрузке; null — вопрос не разобран. */
export function answerGlQuestion(question, agg) {
    if (!agg?.employees?.length) return "Сначала загрузите Excel и нажмите «Посчитать нагрузку».";
    const q = String(question || "").trim();
    if (!q) return null;
    const qn = q.toLowerCase().replace(/ё/g, "е");
    const year = agg.today.getFullYear();
    const day = dateFromQuestion(q, year);
    const dayKey = day ? ymd(day) : null;
    const emp = employeeFromQuestion(q, agg.employees);
    const wantCreated = /поступ|создан|пришл|заведен/.test(qn);
    const byDay = wantCreated ? agg.createdByEmpDay : agg.closedByEmpDay;
    const verb = wantCreated ? "поступило" : "закрыто";
    if (emp && dayKey) {
        return `У ${emp.fio} ${fmtRuDay(day)} ${verb} ${appsPhrase(countOnDay(byDay, emp.fio, dayKey))}.`;
    }
    if (emp) {
        return `У ${emp.fio} в выбранном периоде ${appsPhrase(emp.apps)}.`;
    }
    if (dayKey) {
        return `${fmtRuDay(day)} ${verb} ${appsPhrase(countOnDay(byDay, null, dayKey))} (все сотрудники ГЛ).`;
    }
    if (/сколько|число|всего/.test(qn) && /заяв/.test(qn)) {
        return `В выбранном периоде у сотрудников ГЛ закрыто ${appsPhrase(agg.used)}.`;
    }
    return null;
}
