...
export const PLAN_ARB_PER_WORKDAY = 3.2; // столбец AH = «Арбитраж», 8-часовой день
export const PLAN_L1_PER_WORKDAY = 20; // файл «1-я линия поддержки АС Залоги»
...
export function isFirstLineFile(filename) {
    ...
    return /1[-\s]?я\s*линия|первая\s*линия|ас\s*залоги/i.test(n);
}

export function workUnitsOf(appsNormal, appsArb, appsL1 = 0) {
    return (
        appsNormal / PLAN_APPS_PER_WORKDAY +
        appsArb / PLAN_ARB_PER_WORKDAY +
        appsL1 / PLAN_L1_PER_WORKDAY
    );
}

export function loadPctFromUnits(workUnits, workdays) {
    if (workdays <= 0) return workUnits > 0 ? 999 : 0;
    return (workUnits / workdays) * 100;
}
...
export function aggregateEmployeeLoad(tables, now = new Date()) {
    ...
    for (const table of tables) {
        ...
        const l1File = isFirstLineFile(table.file);
        for (const row of table.rows) {
            ...
            const arb = isArbitrationCell(row[AH_COL]);
            const cur = counts.get(fio) || { apps: 0, appsNormal: 0, appsArb: 0, appsL1: 0 };
            cur.apps += 1;
            if (arb) {
                cur.appsArb += 1;
                usedArb += 1;
    ...
    const employees = [...counts.entries()]
        .map(([fio, c]) => {
            const workUnits = workUnitsOf(c.appsNormal, c.appsArb, c.appsL1);
            const loadPct = loadPctFromUnits(workUnits, workdays);
            return {
                fio,
                apps: c.apps,
                appsNormal: c.appsNormal,
                appsArb: c.appsArb,
                appsL1: c.appsL1,
                workUnits,
                plan,
...
/** План сотрудника: рабочие дни квартала минус отпуск (только пересечение с [qStart, today]). */
export function employeePlan(qStart, today, vacation, planPerDay = PLAN_APPS_PER_WORKDAY) {
    ...
    return { workdays: days, plan: planPerDay * days, vacationDays: off };
}
...
export function withVacations(agg, vacationsByFio) {
    ...
    const employees = agg.employees
        .map((e) => {
            ...
            const p = employeePlan(agg.qStart, agg.today, v, agg.planPerDay);
            const loadPct = loadPctFromUnits(e.workUnits, p.workdays);
            return {
                ...e,
                plan: p.plan,
                workdays: p.workdays,
