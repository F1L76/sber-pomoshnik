...
export const MIN_APPS_FOR_LOAD = 50; // в таблице только сотрудники с заявками сверх этого
...
export const AF_COL = 31; // Excel AF — сегмент
export const AN_COL = 39; // Excel AN — CSI 1–5
export const SEGMENTS = ["ММБ", "КСБ", "ПМЗ", "ЗС", "ПКД", "Прочее"];
...
export function fileKind(filename) {
    ...
    if (/работа\s*с\s*оо|онлайн\s*оценк/.test(n)) return "oo";
...
export function termDaysFromST(sValue, tValue) {
    const s = parseExcelDate(sValue);
    const t = parseExcelDate(tValue);
    if (!s || !t) return null;
    return (t.getTime() - s.getTime()) / 86400000;
}
...
export function segmentFromAF(value) {
    const s = String(value ?? "")
        .toLowerCase()
        .replace(/ё/g, "е");
    if (s.includes("малого")) return "ММБ";
    if (s.includes("корпоративным")) return "КСБ";
    if (s.includes("аналитики")) return "ПМЗ";
    if (s.includes("документарных")) return "ПКД";
    if (s.includes("залог")) return "ЗС";
    return "Прочее";
}

function emptySegCounts() {
    return { ММБ: 0, КСБ: 0, ПМЗ: 0, ЗС: 0, ПКД: 0, Прочее: 0 };
}
...
export function aggregateEmployeeLoad(tables, now = new Date()) {
    ...
    for (const table of tables) {
        ...
        for (const row of table.rows) {
            ...
            if (arb) usedArb += 1;
            if (kind === "l1") usedL1 += 1;
            const days = termDaysFromST(row[S_COL], row[T_COL]);
            if (days != null) {
                cur.termSum += days;
                cur.termN += 1;
            }
            if (isOverdueCell(row[AG_COL])) cur.overdue += 1;
            cur.returns += numberCell(row[W_COL]);
            const bucket = csiBucket(row[AN_COL]);
            if (bucket) cur.csiCounts[bucket - 1] += 1;
            segments[kind][segmentFromAF(row[AF_COL])] += 1;
            counts.set(fio, cur);
            used += 1;
        }
    }
    ...
    const employees = [...counts.entries()]
        .map(([fio, c]) => {
            ...
            return {
                ...
                termSum: c.termSum,
                termN: c.termN,
                avgTermDays: c.termN ? c.termSum / c.termN : null,
                overdue: c.overdue,
                returns: c.returns,
    ...
    return {
        ...
        usedArb,
        usedL1,
        avgTermDays: termN ? termSum / termN : null,
        overdueTotal,
        returnsTotal,
        csiCounts,
        segments,
