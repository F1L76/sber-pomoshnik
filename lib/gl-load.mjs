...
import { resolveVacations } from "./gl-vacations.mjs";

export const HOURS_PER_WORKDAY = 8;
export const MINUTES_PER_HOUR = 60;
export const MIN_OSMOTR = 20; // файл Осмотры
...
/** Имя линии: часть имени файла до «_», без расширения. */
...
/** Минуты на заявку по линии и тегу. */
export function minutesForApp(filename, _fio, ahValue) {
...
export function aggregateEmployeeLoad(tables, now = new Date()) {
    ...
    for (const table of tables) {
        ...
        const line = lineNameFromFile(table.file);
    ...
    const lines = [...lineMap.entries()]
        ...
        .sort((a, b) => b.apps - a.apps || a.name.localeCompare(b.name, "ru"));
    const linesTotal = lines.reduce((s, x) => s + x.apps, 0);
