...
import { getNspdBases, getNspdProxyAuthHeaders, NSPD_REFERER } from "./nspd-config.mjs";

// ponytail: зарубежные DC (Render) часто режут/вешают .gov.ru — без таймаута весь поиск молчит
const NSPD_TIMEOUT_MS = 8_000;
// батч: 12с режет слишком много живых ответов НСПД; интерактивный поиск остаётся на 8с
const GEOCODE_TIMEOUT_MS = Number(process.env.NSPD_GEOCODE_TIMEOUT_MS) || 30_000;
...
async function requestNspdSearchOne(base, query, headers, timeoutMs = NSPD_TIMEOUT_MS) {
    const url = buildNspdSearchUrl(base, query);
    const res = await httpsFetch(url, { headers, timeoutMs });
        ...
        for (const base of bases) {
            requestNspdSearchOne(base, query, headers).then(
                ...
                (err) => {
                    ...
                    left -= 1;
                    if (!settled && left === 0) {
                        const viaProxy = Boolean(process.env.NSPD_BASES?.trim());
                        reject(
                            new Error(
                                viaProxy
                                    ? "НСПД недоступен через NSPD_BASES — проверьте edge-прокси в РФ и NSPD_PROXY_KEY."
                                    : "НСПД (.gov.ru) недоступен с зарубежного сервера (Render). Для Росреестра и границ участка задайте NSPD_BASES=<URL прокси в РФ> и NSPD_PROXY_KEY=<ключ> (scripts/nspd-edge-proxy.mjs)."
                            )
                        );
                    }
                }
...
function geocodeBases() {
    const all = getNspdBases();
    // ponytail: nspd.rosreestr.gov.ru при нагрузке отвечает "OK" без JSON — для батча это ложный хост
    const primary = all.filter((b) => !/nspd\.rosreestr\.gov\.ru/i.test(b));
    return primary.length ? primary : all;
}

/** Только координаты из поиска НСПД — без вкладок и границ (для массового геокода). */
export async function geocodeCadastralCoords(query, { retries = 3, timeoutMs = GEOCODE_TIMEOUT_MS } = {}) {
    const raw = String(query || "").trim();
    if (!raw) {
        return { found: false, cadastralNumber: "", message: "пустой номер", permanent: true };
    }
    let cadastralNumber = raw;
    try {
        cadastralNumber = normalizeCadastralNumber(raw);
    } catch {
        cadastralNumber = raw.replace(/[^\d:]/g, "") || raw;
    }

    const headers = nspdSearchHeaders();
    let lastError = null;
    const attempts = Math.max(1, retries);
    for (let attempt = 0; attempt < attempts; attempt++) {
        for (const base of geocodeBases()) {
            try {
                const result = await requestNspdSearchOne(base, cadastralNumber, headers, timeoutMs);
                const feature = pickFeature(result.features, cadastralNumber);
                if (!feature) {
                    return { found: false, cadastralNumber, message: "не найден в НСПД", permanent: true };
                }
                const object = mapRosreestrObject(feature, cadastralNumber);
                if (!object.location) {
                    return {
                        found: false,
                        cadastralNumber: object.cadastralNumber || cadastralNumber,
                        address: object.address,
                        objectType: object.objectType || object.category,
    ...
    return {
        found: false,
        cadastralNumber,
        message: lastError?.message || "НСПД недоступен",
        permanent: false
    };
}
