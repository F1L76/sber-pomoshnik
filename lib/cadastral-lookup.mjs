...
async function requestNspdSearch(query) {
    ...
    for (const base of NSPD_BASES) {
        ...
        let res;
        try {
            res = await httpsFetch(url, { headers });
        } catch (e) {
            lastError = e;
            continue;
        }
        ...
        if (isNspdNotFoundResponse(res.status, json)) {
            return { found: false, features: [], source: base };
        }

        if (res.ok) {
            const features = json?.data?.features || [];
            return { found: features.length > 0, features, source: base };
        }

        lastError = new Error(`НСПД (${new URL(base).hostname}) вернул HTTP ${res.status}`);
    }

    throw lastError || new Error("Не удалось связаться с НСПД");
}
...
export async function lookupCadastralObject(cadastralNumber) {
    ...
    const { features } = await requestNspdSearch(normalized);
